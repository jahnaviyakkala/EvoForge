...
- [NEW] The system shall provide a high-performance, standards-compliant High Performance application written in C++.
- [NEW] Specification Prompt: "Build a high-performance in-memory Key-Value Cache application in C++
- [NEW] with LRU eviction and TTL expiration. Structure into lru_cache.hpp, lru_cache.cpp, main.cpp, and
- [NEW] tests/test_runner.cpp. Support set(key, value, ttl), get(key), remove(key), clear(), and get_stats()
- [NEW] operations. Include boundary checks for invalid capacity, missing keys, and expired items. main.cpp
- [NEW] must provide an interactive CLI menu loop using std::cin, and tests/test_runner.cpp must contain
- [NEW] automated assertions verifying LRU eviction order, capacity limits, and cache hits/misses."
...
- [NEW] The system shall execute functional logic satisfying: Build a high-performance in-memory Key-Value Cache application in C++
- [NEW] with LRU eviction and TTL expiration. Structure into lru_cache.hpp, lru_cache.cpp, main.cpp, and
- [NEW] tests/test_runner.cpp. Support set(key, value, ttl), get(key), remove(key), clear(), and get_stats()
- [NEW] operations. Include boundary checks for invalid capacity, missing keys, and expired items. main.cpp
- [NEW] must provide an interactive CLI menu loop using std::cin, and tests/test_runner.cpp must contain
