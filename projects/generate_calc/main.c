...
int main(void) {
    ...
    while (printf("\nSelect operation (0-7): ") && scanf("%d", &choice) == 1) {
        ...
        if (choice == 1) printf("Result: %f\n", generate_calc_add(a, b));
        else if (choice == 2) printf("Result: %f\n", generate_calc_subtract(a, b));
        else if (choice == 3) printf("Result: %f\n", generate_calc_multiply(a, b));
        else if (choice == 4) {
            if (generate_calc_divide(a, b, &res)) printf("Result: %f\n", res);
            else printf("Error: Division by zero!\n");
        }
        else if (choice == 5) {
            if (generate_calc_power(a, b, &res)) printf("Result: %f\n", res);
            else printf("Error: Invalid power!\n");
        }
        else if (choice == 6) {
            if (generate_calc_sqrt(a, &res)) printf("Result: %f\n", res);
            else printf("Error: Negative sqrt!\n");
        }
        else if (choice == 7) {
            int n; printf("Enter integer n: ");
            if (scanf("%d", &n) == 1 && generate_calc_factorial(n, &res)) printf("Result: %f\n", res);
